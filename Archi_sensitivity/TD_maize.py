...
# default location and dates (Ouzouer le marche)
_daydate = '2017-07-15'
_timezone = 'Europe/Paris'
_longitude = 1.31
_latitude = 47.54
...
def run_sim(row, **kwds):
    ...
    res = leaf_irradiance(g, **kwds)
    rows_leaves = []
    for i in res.index:
        row_leaf = row.copy(deep=True)
        for k in res:
            row_leaf[k] = res.loc[i][k]
        for arg in kwds:
            row_leaf[arg] = kwds[arg]
        rows_leaves.append(row_leaf)
    return rows_leaves
...
def pool_function(rows, nb_process=2, verbose=True, **kwds):
    ...
    for i, rows in enumerate(pool.imap(function, args)):
        ...
        if rows is not None:
            for row_leaf in rows:
                df = df.append(row_leaf)
